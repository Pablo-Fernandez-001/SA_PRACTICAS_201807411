# Test para crear un restaurante con owner_id incluido

Write-Host "`n=== Testing Restaurant Creation with owner_id ===" -ForegroundColor Cyan
...
Write-Host "`n[2] Creating new restaurant WITH owner_id..." -ForegroundColor Yellow
$restaurantData = @{
    owner_id = 1
    name = "Test Restaurant $(Get-Date -Format 'HHmmss')"
    description = "Test restaurant created via PowerShell with owner_id"
...
try {
    ...
    Write-Host "  Owner ID: $($response.data.ownerId)" -ForegroundColor Green
